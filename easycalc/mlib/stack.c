/*
 *   $Id: stack.c,v 1.41 2008/02/25 12:44:13 cluny Exp $
 * 
...
rpn_pack_record(Trpn item)
{
	...
	packed = MemPtrNew(item.allocsize + sizeof(*packed));
	if (!packed)
		return NULL;
...
rpn_unpack_record(dbStackItem *record)
{
	...
	if (record->datasize) {
		result.u.data = MemPtrNew(record->datasize);
...
rpn_duplicate(Trpn *dest,Trpn source)
{
	*dest = source;
	if (source.allocsize) {
		dest->u.data = MemPtrNew(source.allocsize);
...
stack_get_val(CodeStack *stack,void *arg1,rpntype reqtype)
{
	...
	if (reqtype==string) {
		...
		tmp = MemPtrNew(StrLen(tmpitem.u.stringval)+1);
...
stack_add_val(CodeStack *stack,void *arg,rpntype reqtype)
{
	...
	if (reqtype==real)
		...
		if (((Complex *)arg)->imag == 0.0) {
			...
			value.u.cplxval = MemPtrNew(sizeof(Complex));
			value.allocsize = sizeof(Complex);
			*(value.u.cplxval) = *(Complex *)arg;
		}
	} else if (reqtype == string) {
		value.allocsize = StrLen((char *)arg)+1;
		value.u.stringval = MemPtrNew(value.allocsize);
		StrCopy(value.u.stringval,(char *)arg);
#ifdef SPECFUN_ENABLED
...
stack_new(Int16 count)
{
	...
	result = MemPtrNew(sizeof(*result));
	if (!result)
		return NULL;
	result->stack = MemPtrNew(sizeof(*result->stack)*count);
	if (!result->stack) {
		MemPtrFree(result);
		return NULL;
	}
