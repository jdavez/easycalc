/*
 *   $Id: slist.c,v 1.38 2007/05/31 14:56:31 cluny Exp $
 *
...
list_new(Int16 size)
{
	...
	result = MemPtrNew(sizeof(*result) + size*sizeof(result->item[0]));
	if (result == NULL)
	    return NULL;
...
static char * list_linreg(List *lstx,List *lsty) NEWFUNC;
static char *
list_linreg(List *lstx,List *lsty)
{
	...
	result = MemPtrNew(StrLen(res1)+StrLen(res2)+10);
	StrCopy(result,res2);
	StrCat(result,"x+");
	StrCat(result,res1);
...
list_factor(Functype *func,CodeStack *stack)
{
	...
	fcount = factorize(intarg, factors);
...
list_map(Functype *func,CodeStack *stack)
{
	...
	if (listcount == 1) {
		...
		lsts = MemPtrNew(sizeof(List*) * listcount);
		if (lsts == NULL) 
			return c_memory;
		
		indices = MemPtrNew(sizeof(UInt16) * listcount);
		if (indices == NULL) {
			MemPtrFree(lsts);
			return c_memory;
		}

		values = MemPtrNew(sizeof(Complex) * listcount);
		if (values == NULL) {
			MemPtrFree(lsts);
			MemPtrFree(indices);
			return c_memory;
