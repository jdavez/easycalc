/*
 *   $Id: funcs.cpp,v 1.8 2011/02/28 22:07:47 mapibid Exp $
 *
...
const TCHAR *funcInterDef[] = {
    /* covariance */
    _T("sum((x(1)-mean(x(1)))*(x(2)-mean(x(2))))/dim(x(1))"),
    /* corr. coefficient */
    _T("cov(x(1):x(2))/(stddev(x(1):1)*stddev(x(2):1))"),
    /* qbinomial */
    _T("ibeta(x(1):x(2)-x(1)+1:x(3))"),
    /* qbeta */
    _T("1-ibeta(x(2):x(3):x(1))"),
    /* qchisq */
    _T("1-igamma(x(2)/2:x(1)/2)"),
    /* qF */
    _T("ibeta(x(3)/2:x(2)/2:x(3)/(x(3)+x(2)*x(1)))"),
    /* qPoisson */
    _T("igamma(x(1):x(2))"),
    /* qStudentt */
    _T("0.5(1+sign(x(1))*(ibeta(x(2)/2:0.5:x(2)/(x(2)+x(1)^2))-1)"),
    /* qWeibull */
    _T("exp(-(x(1)/x(2))^x(3))"),
    /* qNormal */
    _T("erfc(x/sqrt(2))/2")
...
{
    rpn->type=complex;
    rpn->u.cplxval = (Complex *) MemPtrNew(sizeof(Complex));
...
// Mapi: added Infinity
static CError inf_func(Trpn *rpn,double val) PARSER;
static CError
inf_func(Trpn *rpn,double val)
{
    double x = 0.0;
...
const struct {
    ...
    {_T("pi"),dnum_func,3.1415926535897932384},
    {_T("e"),dnum_func,2.7182818284590452354},
    {_T("euler"), dnum_func, 0.5772156649015328606},
    {_T("i"),i_func},
    {_T("j"),i_func},
    {_T("Inf"),inf_func},
    {_T("T"),dnum_func,1E12},
    ...
    {_T("u"),dnum_func,1E-6},
    {_T("n"),dnum_func,1E-9},
    {_T("p"),dnum_func,1E-12},
    {_T("f"),dnum_func,1E-15},
    {_T("rand"),rand_func},
    {_T("rNorm"),rnorm_func},
    {_T("now"),time_func},
    {NULL,NULL}
...
rpn_eval_variable(Trpn *dest,Trpn source)
{
    ...
    if (source.type == variable) {
        ...
        *dest=tmp;
        ...
        CodeStack *tstack;
        Functype *func= (Functype *) MemPtrNew(sizeof(*func));
...
set_ans_var(Trpn ans)
{
    ...
    err = db_write_variable(_T("ans"),ans);
    if (!err)
...
change_param_name(Functype *func,CodeStack *stack)
{
    ...
    if (StrLen(name)>MAX_FUNCNAME)
        return c_badarg;
    oldname = (TCHAR *) MemPtrNew((StrLen(parameter_name)+1)*sizeof(TCHAR));
    ...
    tmp.allocsize=(StrLen(oldname)+1)*sizeof(TCHAR);
    stack_push(stack,tmp);
...
exec_function(CodeStack *funcstack,CodeStack *varstack, Int16 paramcount)
{
    ...
    argarr = (Trpn *) MemPtrNew(sizeof(*argarr) * paramcount);
 ...
 ***********************************************************************/
CError
text_function(TCHAR *name,Int16 paramcount,CodeStack *stack)
{
    CError err;
    CodeStack *newstack;
...
func_interdef(Functype *func,CodeStack *stack)
{
    ...
    myfunc = text_to_stack((TCHAR *)funcInterDef[func->num],&err);
    if (err)
        return err;
...
convert_cplx_to_string(Functype *func,CodeStack *stack)
{
    ...
    if (func->num == FUNC_GONIO) {
        res = (TCHAR *) MemPtrNew((MAX_FP_NUMBER*2+10)*sizeof(TCHAR));
        ...
        res = (TCHAR *) MemPtrNew((MAX_FP_NUMBER*2+20)*sizeof(TCHAR));
        fp_print_double(res,tmp.r);
        StrCat(res,_T("(cos("));
        fp_print_double(res+StrLen(res),tmp.angle);
        StrCat(res,_T(")+i*sin("));
...
convert_real_to_string(Functype *func,CodeStack *stack)
{
    ...
    res = (TCHAR *) MemPtrNew((MAX_FP_NUMBER+20)*sizeof(TCHAR));
    tmp = res;
    if (func->num==FUNC_RADIAN) {
        // Mapi: take calculator mode in account for conversion
        // Allows to convert from one unit to another and display result.
        // Also reduce to the range ]-2pi,2pi[.
        double conv;
        if (calcPrefs.trigo_mode == degree) {
            conv = fmod(arg/180.0, 2.0);
        } else if (calcPrefs.trigo_mode == radian) {
            conv = fmod(arg/M_PIl, 2.0);
        } else { // Grads
            conv = fmod(arg/200.0, 2.0);
        }
        fp_print_double(res, conv);
        StrCat(res,_T("pi"));
    }
    else if (func->num==FUNC_DEGREE || func->num == FUNC_DEGREE2) {
        // Mapi: take calculator mode in account for conversion
        // Allows to convert from one unit to another and display result.
        // Also reduce to the range ]-360,360[.
        double conv;
        if (calcPrefs.trigo_mode == degree) {
            conv = fmod(arg, 360.0);
        } else if (calcPrefs.trigo_mode == radian) {
            conv = fmod(arg/M_PIl*180.0, 360.0);
        } else { // Grads
            conv = fmod(arg/10.0*9.0, 360.0);
        }
        if (conv<0) {
            *tmp++=_T('-');
            conv=-conv;
         ...
         */
        fp_print_double(tmp,trunc(conv+1E-12));
        tmp+=StrLen(tmp);
        *tmp++=_T('°');
        if (conv-trunc(conv)<1E-15)
            StrCopy(tmp,_T("0\'"));
        else {
            if (func->num == FUNC_DEGREE) {
                fp_print_double(tmp,trunc(60*(conv-trunc(conv+1E-12))+1E-12));
                tmp+=StrLen(tmp);
                *tmp++=_T('\'');
                fp_print_double(tmp,60*(conv*60-trunc(conv*60+1E-12)));
            } else { /* FUNC_DEGREE2 */
                fp_print_double(tmp,60*(conv-trunc(conv+1E-12))+1E-12);
                StrCat(tmp,_T("'"));
            }
        }
    }
    else if (func->num==FUNC_GRAD) {
        // Mapi: new function.
        // Take calculator mode in account for conversion
        // Allows to convert from one unit to another and display result.
        // Also reduce to the range ]-400,400[.
        double conv;
        if (calcPrefs.trigo_mode == degree) {
            conv = fmod(arg/9.0*10.0, 400.0);
        } else if (calcPrefs.trigo_mode == radian) {
            conv = fmod(arg/M_PIl*200.0, 400.0);
        } else { // Grads
            conv = fmod(arg, 400.0);
        }
        if (conv<0) {
