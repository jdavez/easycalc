/*
 *   $Id: mathem.cpp,v 1.2 2009/12/15 21:37:44 mapibid Exp $
 * 
...
// Mapi: rewrite for better calculation precision
...
// Mapi: bug correction (on global sign of formula)
#define FIN_FV_BEGIN(I,N,PV,PMT,PYR) (-((-(PV)-(PMT)*((PYR)*100.0/(I)+1.0))*pow(1.0+(I)/100.0/(PYR),(N))+(PMT)*((PYR)*100.0/(I)+1.0)))
#define FIN_FV_END(I,N,PV,PMT,PYR)   (-((-(PV)-(PMT)*(PYR)*100.0/(I))*pow(1.0+(I)/100.0/(PYR),(N))+(PMT)*(PYR)*100.0/(I)))
#define FIN_FV_ZERO(N,PV,PMT)        (-(-(PV)-(PMT)*(N)))
...
// Mapi: bug correction (on global sign of formula)
#define FIN_PV_BEGIN(I,N,PMT,FV,PYR) (-((-(FV)-(PMT)*((PYR)*100.0/(I)+1.0))/pow(1.0+(I)/100.0/(PYR),(N))+(PMT)*((PYR)*100.0/(I)+1.0)))
#define FIN_PV_END(I,N,PMT,FV,PYR)   (-((-(FV)-(PMT)*(PYR)*100.0/(I))/pow(1.0+(I)/100.0/(PYR),(N))+(PMT)*(PYR)*100.0/(I)))
// Mapi: bug correction (on sign of FV)
#define FIN_PV_ZERO(N,PMT,FV)        (-(-(FV)+(PMT)*(N)))
...
// Mapi: bug correction (on sign of FV)
#define FIN_PMT_BEGIN(I,N,PV,FV,PYR) (((FV)-(PV)*pow(1.0+(I)/100.0/(PYR),(N)))/(pow(1.0+(I)/100.0/(PYR),(N))-1.0)*(I)/(100.0*(PYR)+(I)))
#define FIN_PMT_END(I,N,PV,FV,PYR)   (((FV)-(PV)*pow(1.0+(I)/100.0/(PYR),(N)))/(pow(1.0+(I)/100.0/(PYR),(N))-1.0)*(I)/100.0/(PYR))
#define FIN_PMT_ZERO(N,PV,FV)        (-((PV)-(FV))/(N))

// Mapi: bug correction (on sign of PMT)
...
// Mapi: bug correction (on sign of PV)
#define FIN_N_BEGIN(I,PV,PMT,FV,PYR) (log((-(FV)-(PMT)*((PYR)*100.0/(I)+1.0))/(-(PV)-(PMT)*((PYR)*100.0/(I)+1.0)))/log(1.0+(I)/100.0/(PYR)))
#define FIN_N_END(I,PV,PMT,FV,PYR)   (log((-(FV)-(PMT)*(PYR)*100.0/(I))/(-(PV)-(PMT)*(PYR)*100.0/(I)))/log(1.0+(I)/100.0/(PYR)))
// Mapi: bug correction (on sign of FV)
#define FIN_N_ZERO(PV,PMT,FV)        (-((PV)-(FV))/(PMT))
...
math_financial(Functype *func,CodeStack *stack)
{
	...
	case MATH_FINPMT:
		if (arg[0] == 0.0)
			result = FIN_PMT_ZERO(arg[1],arg[2],arg[3]);
		else if (begin)
	...
	case MATH_FINPV:
		if (arg[0] == 0.0)
			result = FIN_PV_ZERO(arg[1],arg[2],arg[3]);
		else if (begin)
	...
	case MATH_FINN:
		if (arg[0] == 0.0)
			result = FIN_N_ZERO(arg[1],arg[2],arg[3]);
		else if (begin)
	...
	case MATH_FINFV:
		if (arg[0] == 0.0)
			result = FIN_FV_ZERO(arg[1],arg[2],arg[3]);
		else if (begin)
...
math_phi(Functype *func,CodeStack *stack)
{
	...
	if (value == 0) {
		...
		fcount = factorize(value, (Int32 *) factors);
		for(i=0; i<fcount; i++) {
