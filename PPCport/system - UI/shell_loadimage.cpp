 ...
 *****************************************************************************/
/* shell_loadimage.cpp : load a GIF file.
 *****************************************************************************/
...
int shell_loadimage(Skin *pskin, HDC hdc) {
    ...
    while (1) {
        ...
        if (whatnext == ',') {
            ...
            if ((info & 128) == 0) {
                ...
                lbpp = (info & 7) + 1;
                lncolors = 1 << lbpp;
                lcmap = (SkinColor *) mmalloc(256 * sizeof(SkinColor));
                ...
                if (pm->depth != 24) {
                    ...
                    unsigned char *newpixels = (unsigned char *)
                        mmalloc(newbytesperline * pm->height);
                        // TODO - handle memory allocation failure
                            ...
                            if (curr_code == end_code) {
                                ...
                                for (j=explen-1 ; j>=0 ; j--) {
                                    ...
                                    if (pm->depth == 8)
                                        ...
                                        if (pixel)   pm->pixels[index] |= mask;
                                        else   pm->pixels[index] &= ~mask;
                                    }
                                    if (++h == iwidth) {
                                        h = 0;
                                        if (interlaced) {
    ...
    if (lcmap != NULL && lcmap != pm->cmap)   mfree(lcmap);
    if (pm->depth == 24) {
        mfree(pm->cmap);
        pm->cmap = NULL;
    }
    ...
    if (pm->depth == 1)   type = IMGTYPE_MONO;
    else if (pm->depth == 8)   type = IMGTYPE_COLORMAPPED;
    else /* pm->depth == 24 */
        type = IMGTYPE_TRUECOLOR;

    res = skin->init_image(type, 256, pm->cmap, pm->width, pm->height);
    if (res) {
        ptr = pm->pixels;
